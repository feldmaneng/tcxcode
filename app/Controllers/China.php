...
public function company4667227()
{
	...
	//$this->grocery_crud->display_as('StaffID','Exhibitor Staff');
...
public function contact585442()
{
	...
	//$this->grocery_crud->set_subject('Guest 来宾');
	...
	$this->grocery_crud->set_rules('CN_Company', 'Chinese Company Name<br>公司名称（中文）', 'callback_companyVerify[Company]');
	$this->grocery_crud->set_rules('GivenName', 'Given (First) Name 名（英文）', 'callback_givenNameVerify[ChineseName]');
	$this->grocery_crud->set_rules('FamilyName', 'Family (Last) Name 姓（英文）', 'callback_familyNameVerify[ChineseName]');
	$this->grocery_crud->set_rules('ChineseName', 'Chinese Name 名', 'callback_givenNameVerify[FamilyName]');
	$this->grocery_crud->set_rules('Phone', 'Work Phone 单位电话', 'callback_phoneVerify[Mobile]');
	$this->grocery_crud->set_rules('Mobile', 'Mobile Phone 手机', 'callback_phoneVerify[Phone]');
...
/*
   ...
   /*	echo "<pre>";
   	...
   	echo "</pre>";
   	*/
   	$this->grocery_crud->where('InvitedByCompanyID',$companyID);
   	$this->grocery_crud->where('EventYear', EventYear); 
	$this->grocery_crud->set_theme('datatables');
	$this->grocery_crud->set_subject('Guest 来宾');
	...
	$this->grocery_crud->columns ('Email','GivenName','FamilyName','ChineseName','Company','CN_Company');
	$this->grocery_crud->fields ('Email','GivenName','FamilyName', 'InvitedByCompanyID', 'EventYear', //'BanquetCompanyID',
	...
	$this->grocery_crud->set_rules('GivenName', 'Given (First) Name 名（英文）', 'callback_givenNameVerify[ChineseName]');
	$this->grocery_crud->set_rules('FamilyName', 'Family (Last) Name 姓（英文）', 'callback_familyNameVerify[ChineseName]');
	$this->grocery_crud->set_rules('ChineseName', 'Chinese Name（中文）', 'callback_familyNameVerify[FamilyName]');
	$this->grocery_crud->set_rules('Phone', 'Work Phone 单位电话', 'callback_phoneVerify[Mobile]');
	$this->grocery_crud->set_rules('Mobile', 'Mobile Phone 手机', 'callback_phoneVerify[Phone]');
	...
	$this->grocery_crud->field_type('EventYear','hidden', EventYear);
	$this->grocery_crud->field_type('BanquetCompanyID','hidden');
	$this->grocery_crud->field_type('Invited','hidden');
	$this->grocery_crud->field_type('ToPrint','hidden','Yes'); // if we've edited it or added it we should set it to print
	...
	// Force a refresh after a delete in case the number of guests falls below the guest 
	// limit so the add button is shown again	
	$this->grocery_crud->set_lang_string('delete_success_message',
		 'Your data has been successfully deleted from the database.<br/>Please wait while you are redirecting to the list page.\\n已从数据库里成功删除您的数据。正在返回列表，请稍后
		 <script type="text/javascript">
		  window.location = "'.site_url(strtolower(__CLASS__).'/'.strtolower(__FUNCTION__)).'";
		 </script>
