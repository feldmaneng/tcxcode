...
//this determines how many rows the sheet has
		...
		$row2 = $query2->getRow();
			...
			$SESSION=$results[$n]["Session"];
				...
				if($event == 'Mesa'){
					$pdf->Image($_SERVER["DOCUMENT_ROOT"].'/images/TestConXletterframe2XXX.png', 0, 0, 280, 210, 'PNG', '', '',true,300, '', false, false, 0, false, false, false);
						...
						$pdf->SetFont('times', '', 20);
						 ...
						 if(!empty($row2->Nickname)){
							 if($FIRSTNAME == $NICKNAME){
								$pdf->MultiCell(150, 25,$FIRSTNAME." ".$LASTNAME, 0, 'C', 0, 0, 62.5, $y+2.5*$z, true);
								}
								else{
							 $pdf->MultiCell(150, 25,$FIRSTNAME." ".'"'.$NICKNAME.'" '.$LASTNAME, 0, 'C', 0, 0, 62.5, $y+2.5*$z, true);
								}
						 }
						 else{
							$pdf->MultiCell(150, 25,$FIRSTNAME." ".$LASTNAME, 0, 'C', 0, 0, 62.5, $y+2.5*$z, true);
				...
				if($event == 'China'){
					$pdf->Image($_SERVER["DOCUMENT_ROOT"].'/images_new/TestConXChinaOrangeFrameA4r4.png', 0, 0, 300, 205, 'PNG', '', '',true,300, '', false, false, 0, false, false, true);
				...
				if($event == 'Korea'){
					...
					else if($SESSION == 'Panel' || $SESSION == 'Panel2'|| $SESSION == 'Panels')
					{
					$pdf->MultiCell(100, 25,"for the Panel", 0, 'C', 0, 0, $x,$y+3.9*$z, true);
					} else
				   {
				   $pdf->MultiCell(100, 25,"for the presentation", 0, 'C', 0, 0, $x,$y+3.9*$z, true);
				   }
